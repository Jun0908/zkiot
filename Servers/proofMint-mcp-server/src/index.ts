...
function getProvider(): JsonRpcProvider { return new JsonRpcProvider(need('RPC_URL')); }
function getWallet(): Wallet { return new Wallet(need('WALLET_PRIVATE_KEY'), getProvider()); }
function getContract(runner?: ethers.Signer | JsonRpcProvider): Contract {
...
/* ---------- utils ---------- */
function hex32(x: string): `0x${string}` {
  if (!x.startsWith('0x')) throw new Error('expected 0x-prefixed hex');
  if (x.length === 66) return x as `0x${string}`;
  if (x.length < 66) return zeroPadValue(x, 32) as `0x${string}`;
  throw new Error('hex32 too long');
}
function now(): number { return Math.floor(Date.now() / 1000); }
function randomNonce32(): `0x${string}` { return hexlify(randomBytes(32)) as `0x${string}`; }

/** keccak256( abi.encode(commitRoot, keccak256(period), keccak256(device)) ) */
function computeTokenId(commitRoot: `0x${string}`, period: string, device: string): bigint {
...
function buildDomain(chainId: bigint, verifyingContract: string) {
...
type VoucherInput = z.infer<typeof VoucherZod>;

type MintVoucherStruct = {
  to: string;
  tokenId: bigint;
  commitRoot: `0x${string}`;
  vkHash: `0x${string}`;
  schema: `0x${string}`;
  ...
  for (const log of receipt.logs) {
    try {
      ...
      if (parsed?.name === 'Minted' || parsed?.name === 'Transfer') {
        const tid = parsed.args?.tokenId as bigint | undefined;
...
server.registerTool(
  ...
  async (raw, _extra) => {
    const input = VoucherZod.parse(raw) as VoucherInput;
    const provider = getProvider();
    ...
    const periodHash = keccak256(toUtf8Bytes(input.period)) as `0x${string}`;
    const deviceHash = keccak256(toUtf8Bytes(input.device)) as `0x${string}`;
    const tokenId =
    ...
    const nonce = (input.nonce ?? randomNonce32()) as `0x${string}`;
    const tokenURI = input.tokenURI ?? '';

    const voucher: MintVoucherStruct = {
      to,
      tokenId,
      commitRoot,
      vkHash,
    ...
    const signature = await wallet.signTypedData(domain, TypedDataTypes as any, voucher);
    const digest = ethers.TypedDataEncoder.hash(domain, TypedDataTypes as any, voucher);
...
server.registerTool(
  ...
  async ({ voucher, signature }, _extra) => {
    ...
    const v: MintVoucherStruct = {
      to: getAddress(voucher.to),
      tokenId: BigInt(voucher.tokenId),
      commitRoot: voucher.commitRoot as `0x${string}`,
      vkHash: voucher.vkHash as `0x${string}`,
      schema: voucher.schema as `0x${string}`,
      periodHash: voucher.periodHash as `0x${string}`,
      deviceHash: voucher.deviceHash as `0x${string}`,
...
server.registerTool(
  ...
  async ({ commitRoot, period, device }, _extra) => {
    const id = computeTokenId(commitRoot as `0x${string}`, period, device);
    const periodHash = keccak256(toUtf8Bytes(period));
    const deviceHash = keccak256(toUtf8Bytes(device));

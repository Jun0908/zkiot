...
function extractTransfer(receipt) {
...
function extractMinted(receipt) {
    for (const log of receipt.logs) {
        try {
            const parsed = iface.parseLog(log);
...
// ==== A) 自分にファウセット（mint()）====
server.registerTool('mint_simple', {
...
// ==== D) 残高表示（balanceOf）====
server.registerTool('balance_of', {
    title: 'Show balance',

...
// ENS: 逆引き → 正引きで検証（推奨手順）
async function resolveEnsPrimary(addr, opts) {
    ...
    try {
        ...
        console.error('[ENS] reverse lookup failed:', e.message);
    }
    ...
    if (name) {
        try {
            ...
            console.error('[ENS] forward lookup error:', e.message);
        }
    }
    // 3) ENSが無い → アドレスで返す
...
server.registerTool('agent_ens_name', {
    title: 'Get ENS primary name (and optionally avatar) for an address',
    description: 'Returns the ENS primary name (reverse → verified by forward). ' +
    ...
    // 2) ensName が来た or address に .eth が入っている → 正引きでアドレス取得
        ...
        try {
            ...
            checksum = getAddress(fwd);
        }
        catch (e) {
            return { content: [{ type: 'text', text: `❌ ENS forward lookup failed: ${e.message}` }] };
        }
    }
    ...
    try {
        ...
        return { content: [{ type: 'text', text: `❌ ENS lookup failed: ${e.message}` }] };
    }
});

...
const VERIFIER_ABI: InterfaceAbi = [
  {
    inputs: [
...
server.registerTool(
  'verify_groth16',
  {
    ...
    description:
      'Calls Groth16Verifier.verifyProof(_pA,_pB,_pC,_pubSignals). ' +
  ...
  async (args) => {
    ...
    const ok: boolean = await contract.verifyProof(a, b, c, pubSignals);

    // 2) Optional: compute hashes
    let callDataHash: string | undefined;
    ...
    if (withHashes) {
      if (callDataHash) lines.push(`callDataHash=${callDataHash}`);
      if (proofHash) lines.push(`proofHash=${proofHash}`);
      if (chainIdStr) lines.push(`chainId=${chainIdStr}`);
